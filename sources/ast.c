...
static int	parse_pipeline(t_list **lexme_ptr, t_ast_node *ast)
{
	...
	while (*lexme_ptr != NULL)
	{
		...
		else 
		{
			if (ac + rc + pc == 0)
				break;
			return (ft_dprintf(2, ERROR_MSG"(2) error near unexpected \
...
static int	parse_pipelinelist(t_list **lexme_ptr, t_ast_node *ast, int init)
{
	...
	if (*lexme_ptr == NULL)
		return (ft_dprintf(2, ERROR_MSG"(4) missing tokens\n"), -1);
	while (*lexme_ptr != NULL)
	{
		...
		if (t == AND || t == OR)
		{
			if ((*lexme_ptr)->next == NULL)
				return (ft_dprintf(2, ERROR_MSG"(6) missing token after `%s' \
					token\n", _type_str(*lexme_ptr)), -1);
			...
			return (ft_dprintf(2, ERROR_MSG"(7) syntax error near unxepected \
				`%s' token\n", _type_str(*lexme_ptr)), -1);
		*lexme_ptr = (*lexme_ptr)->next;
	}
	return (0);
...
t_ast_node	*parse(t_list *lexemes)
{
	...
	lexme = *lexme_ptr;
	if (lexme && lexme->next)
		return (ft_dprintf(2, ERROR_MSG"(8) syntax error near unexpected \
